...
#define USB_COMPOSITE_CONFIG_DESC_SIZ       (9 + (USB_HID_CONFIG_DESC_SIZ - 9) + (USB_MIDI_CONFIG_DESC_SIZE - 9))
...
static uint8_t  USBD_COMPOSITE_Setup (USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
	...
	if(((req->bmRequest & USB_REQ_RECIPIENT_MASK) == USB_REQ_RECIPIENT_INTERFACE && req->wIndex == COMPOSITE_HID_INTERFACE_IDX) ||
		((req->bmRequest & USB_REQ_RECIPIENT_MASK) == USB_REQ_RECIPIENT_ENDPOINT && ((req->wIndex & 0x7F) == COMPOSITE_HID_EP_IDX)))
	{
